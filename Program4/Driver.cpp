...
int main(int argc, char* argv[])
{
	...
	Mat mustache = imread("mustache.png");
	...
	while (true) {
		...
		Mat lilImage = imread("mustache.jpeg", IMREAD_COLOR);
		resize(lilImage, lilImage, Size(), 0.25, 0.25);

		for (int r = 0; r < lilImage.rows; r++) {
			for (int c = 0; c < lilImage.cols; c++) {
				frame.at<Vec3b>(r, c)[0] = lilImage.at<Vec3b>(r, c)[0];
				frame.at<Vec3b>(r, c)[1] = lilImage.at<Vec3b>(r, c)[1];
				frame.at<Vec3b>(r, c)[2] = lilImage.at<Vec3b>(r, c)[2];
			}
		}
	...
	IplImage *img5 = cvLoadImage("mustache.png");

	IplImage *img6 = cvLoadImage("background.jpg");

	//cvShowManyImages("Image", 2, img5, img6);

	Mat bigImage = imread("background.jpg",IMREAD_COLOR);
	Mat lilImage = imread("mustache.jpeg", IMREAD_COLOR);
	...
	for (int r = 0; r < lilImage.rows; r++) {
		for (int c = 0; c < lilImage.cols; c++) {
			bigImage.at<Vec3b>(r, c)[0] = lilImage.at<Vec3b>(r, c)[0];
			bigImage.at<Vec3b>(r, c)[1] = lilImage.at<Vec3b>(r, c)[1];
			bigImage.at<Vec3b>(r, c)[2] = lilImage.at<Vec3b>(r, c)[2];
		}
	}*/
